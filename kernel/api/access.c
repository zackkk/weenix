...
int range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
	...
	/*
	uint32_t curr_addr = (uint32_t) vaddr;
	curr_vfn = ((curr_addr - (curr_addr % PAGE_SIZE))/ PAGE_SIZE) + 1;
	
	addrArea = vmmap_lookup(p->p_vmmap, curr_vfn);
	KASSERT(addrArea);
